...
<title>${products.titulo }-Casa do Código</title>
<!-- 
	...
	<article id="${products.id }">
		<header id="product-highlight" class="clearfix">
			<div id="product-overview" class="container">
				...
				<h1 class="product-title">${products.titulo }</h1>
				<p class="product-author">
					<span class="product-author-link"> </span>
				</p>
				<p class="book-description">${products.descricao }</p>
			</div>
		</header>
		...
		<section class="buy-options clearfix">
			<form action='<c:url value="/carrinho/add" />'  method="post" class="container">
				...
				<ul id="variants" class="clearfix">
					...
					<c:forEach items="${products.listPreco }" var="preco">
					
						<li class="buy-option" itemprop="offers"
							itemtype="http://schema.org/Offer"><input type="radio"
							name="tipoPreco" class="variant-radio" id="tipoPreco"
							value=${preco.tipoPreco } checked> <label
							itemprop="category" class="variant-label">${preco.tipoPreco }</label>
							<p class="variant-price" itemprop="price">${preco.valor }</p></li>
					</c:forEach>
				</ul>
				<button type="submit" class="submit-image icon-basket-alt"
					title="Compre Agora ${products.titulo }!">COMPRE JÁ!</button>
			</form>
		</section>
		...
		<div class="container">
			...
			<section class="data product-detail">
				...
				<p>
					Número de páginas: <span>${products.paginas }</span>
				</p>
				<p>Data de publicação: <span class="publishedAt">
					<fmt:formatDate pattern="dd/MM/yyyy" value = "${products.dataLancamento.time }"/></span></p>
				<p>
